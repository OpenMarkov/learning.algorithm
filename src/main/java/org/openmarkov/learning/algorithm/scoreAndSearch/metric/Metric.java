...
public abstract class Metric implements PNUndoableEditListener {
	...
	protected double score(AddLinkEdit edition, boolean change) {
		...
		Node destinationNode = probNet.getNode(edition.getVariable2());
		Node originNode = probNet.getNode(edition.getVariable1());
	...
	protected double score(InvertLinkEdit edition, boolean change) {
		Node initialDestinationNode = probNet.getNode(edition.getVariable2());
		Node initialOriginNode = probNet.getNode(edition.getVariable1());
		double lastNodeScore;
		double newNodeScore;
		double result;
		...
		newNodeScore = scoreNode(initialDestinationNode, absFrequencies, false);
		/*
		 * If change is true it's because we have to update the probNet values
		 * and store the node dimension and entropy to avoid repeating the
		 * calculations
