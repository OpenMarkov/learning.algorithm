...
public class StatisticalUtilities {
	...
	static public double gammp(double a, double x) {
		if (x < 0.0 || a <= 0.0) {
			throw new IllegalArgumentException("Invalid parameters: 'a' must be > 0 and 'x' must be >= 0");
		}
		if (x < (a + 1.0)) {
	...
	static public double gser(double a, double x) {
		...
		if (x <= 0.0) {
			if (x < 0.0)
				throw new IllegalArgumentException("Error: Parameter x is less than 0.0 in routine gser in LogFactorial");
			gammser = 0.0;
			return gammser;
		} else {
	...
	static public double gammaLn(double xx) {
		if (xx <= 0) {
			throw new IllegalArgumentException("Parameter 'xx' must be > 0 in 'gammaln'.");
		}
